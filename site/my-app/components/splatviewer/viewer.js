...
async function main(splatUrl) {
    ...
    console.log("[Viewer] URL type:", typeof url, "Is string:", typeof url === 'string');
    ...
    // Download progress overlay elements (injected by React page)
    ...
    while (true) {
        ...
        // Update centered download progress (bytes)
        try {
            if (downloadFill && splatData.length) {
                const percentBytes = Math.min(100, Math.floor((bytesRead / splatData.length) * 100));
